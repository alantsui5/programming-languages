...
BinaryTreeADT RightLeftRotate(BinaryTreeADT t){
    return LeftRotate(NonemptyBinaryTree(Root(t),
                                  LeftSubtree(t),
...
BinaryTreeADT LeftRightRotate(BinaryTreeADT t){
    return RightRotate(NonemptyBinaryTree(Root(t),
...
if (sign < 0) {
    ...
    if (AVLTreeHeight(LeftAVLSubtree(NewTree)) – AVLTreeHeight(RightAVLSubtree(NewTree)) == 2)
        return (strcmp(GetNodeKey(X), GetNodeKey(AVLRoot(LeftAVLSubtree(NewTree)))) < 0 ? RightRotate(NewTree) : LeftRightRotate(NewTree));
    return NewTree;
}
if (sign > 0) {
    AVLTreeADT NewTree = NonemptyAVLTree(AVLRoot(T),
    ...
    if (AVLTreeHeight(RightAVLSubtree(NewTree)) – AVLTreeHeight(LeftAVLSubtree(NewTree)) == 2)
        return (strcmp(GetNodeKey(X), GetNodeKey(AVLRoot(RightAVLSubtree(NewTree)))) > 0 ? LeftRotate(NewTree) :RightLeftRotate(NewTree));
    return NewTree;
    }
}
